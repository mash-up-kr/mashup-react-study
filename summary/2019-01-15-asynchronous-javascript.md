...
그 이유는 `$.get(/* ... */)`이 데이터 요청 작업을 비동기적으로 처리하기 때문이다. 즉, 서버에 데이터를 요청 후 응답을 받기까지 다음 작업의 수행을 중지하는 것이 아니라, 해당 요청에 대한 처리를 브라우저에 위임한 후 10행에서 로그를 출력하는 작업을 먼저 수행했기 떄문에 위와 같은 결과를 출력했던 것이다. 응답받은 데이터를 `issues` 변수에 할당하는 작업은 그 이후에 이뤄진다.
